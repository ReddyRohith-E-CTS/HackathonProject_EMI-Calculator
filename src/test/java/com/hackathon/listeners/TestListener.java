...
// TestNG listener that cleans stale artefacts before each suite run, flushes the Extent report after the suite, and logs every test lifecycle event via Log4j2.
public class TestListener implements ITestListener, ISuiteListener {
    ...
    // reports/extent/ is intentionally NOT cleaned - every Extent report has a unique timestamp, so runs accumulate side-by-side without overwriting.
    @Override
    public void onStart(ISuite suite) {
        cleanFolder(SCREENSHOTS_DIR, name -> name.toLowerCase().endsWith(".png"));
        cleanFolder(ALLURE_RESULTS_DIR, name -> true);
        cleanFolder(CUCUMBER_REPORTS_DIR,
                name -> (name.endsWith(".html") || name.endsWith(".json")) && name.contains("cucumber"));
    ...
    public void onStart(ITestContext c) {
        log.info("Context START: {}", c.getName());
    }
